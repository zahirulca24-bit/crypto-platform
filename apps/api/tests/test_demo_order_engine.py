...
def services(db_session):
    risk_store = RiskDecisionStore(db_session)
    decision = RiskEngine(risk_store).evaluate(approved_request())
    return decision, risk_store, DemoOrderStore(db_session)
...
def test_rejected_risk_decision_is_blocked(db_session):
    ...
    with pytest.raises(RiskDecisionNotApproved):
        DemoOrderEngine(risk_store, DemoOrderStore(db_session)).submit(DemoOrderSubmitRequest(risk_decision_id=rejected.id))
